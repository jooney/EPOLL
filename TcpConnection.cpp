...
TcpConnection::TcpConnection(EventLoop* loop,
	...
	:_channel(new Channel(loop,sockfd,log)),
	_loop(loop),
	_name(nameArg),
	_state(kConnecting),
	_reading(true),
...
//	LOG_DEBUG << "TcpConnection::TcpConnection "<< this <<"loop: "<< loop;
	_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"loop[%p]"),loop);
...
{
	_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"sendInLoopEX"));
//	LOG_INFO << "TcpConnection::sendInLoopEx"; 
	...
	{
		_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"disconnected, give up writing"));
//		LOG_INFO << "disconnected, give up writing";
	...
	if (!_channel->isWriting() && _outputBuffer.readableBytes())
	{
		...
		_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"sendInLoopEX [%d] bytes"),nwrote);
	//	LOG_INFO << "TcpConnection::sendInLoopEx: "<< nwrote << "bytes"; 
		...
		//		LOG_SYSERR << "TcpConnection::sendInLoop";
				_log(ZQ::common::Log::L_ERROR,CLOGFMT(TcpConnection,"sendInLoopEX"));
	...
	{
		_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"sendInLoopEX remaining[%d]"),remaining);
...
void TcpConnection::connectEstablished()
{
	...
	_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"connectEstablished"));
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
	...
	{
		errno = savedErrno;
		_log(ZQ::common::Log::L_ERROR,CLOGFMT(TcpConnection,"handleRead"));
...
void TcpConnection::handleWrite()
{
	...
	if (_channel->isWriting())
	{
		...
		{
			_log(ZQ::common::Log::L_ERROR,CLOGFMT(TcpConnection,"handleWrite"));
...
{
	_loop->assertInLoopThread();
	_log(ZQ::common::Log::L_DEBUG,CLOGFMT(TcpConnection,"handleClose() fd[%d] state[%s]"),_channel->fd(), stateToString());
...
void TcpConnection::handleError()
{
	...
	_log(ZQ::common::Log::L_ERROR,CLOGFMT(TcpConnection,"handleError()"));
